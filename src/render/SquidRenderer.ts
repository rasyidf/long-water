/**
 * Procedural deep-water squid, one filled silhouette per animal into the
 * `predators` layer. Geometry is built in the squid's local frame — `+along` is
 * the mantle tip, arms fan back off the head — then rotated by `sq.heading` and
 * projected to screen. The mantle jets (pulsing width), the arms sway when
 * cruising and flare/curl forward as `sq.flare` rises for a grab, and the skin
 * flushes from deep maroon to a pale ghost with `sq.arousal`.
 *
...
export class SquidRenderer implements System {
  ...
  render(ctx: GameContext): void {
    ...
    for (const sq of ctx.squid.squids) {
      ...
      this.drawSquid(g, sq, cam.sx(sq.x), cam.sy(sq.y), cam.scale);
    }
  }
  ...
  private drawSquid(
    ...
    // ---- caudal fins near the mantle tip ----------------------------------
    {
      ...
      const f1 = ml * 0.98;
      const span = mw * 1.9;
      g.moveTo(X(f0, mw * 0.5), Y(f0, mw * 0.5));
      g.quadraticCurveTo(
        X(f1 * 0.9, span),
        Y(f1 * 0.9, span),
        X(f1, 0),
      ...
      g.closePath();
      g.fill({ color: finCol, alpha: 0.9 });
    }

    // ---- mantle -----------------------------------------------------------
    g.moveTo(X(0, -mw * 0.62), Y(0, -mw * 0.62));
    g.quadraticCurveTo(
      X(ml * 0.22, -mw),
      Y(ml * 0.22, -mw),
      X(ml * 0.6, -mw * 0.62),
    ...
    g.closePath();
    g.fill({ color: skin, alpha: 0.95 });

    // pale mantle stripe (countershade)
    g.moveTo(X(0, mw * 0.1), Y(0, mw * 0.1));
    g.quadraticCurveTo(
      X(ml * 0.5, mw * 0.66),
      Y(ml * 0.5, mw * 0.66),
      X(ml * 0.92, mw * 0.12),
    ...
    g.closePath();
    g.fill({ color: mixColor(skin, 0xffffff, 0.3), alpha: 0.35 });

    // ---- head + eye -----------------------------------------------------
    g.circle(X(-mw * 0.1, 0), Y(-mw * 0.1, 0), mw * 0.72 * k);
    g.fill({ color: skin, alpha: 0.95 });

    const er = mw * 0.34 * k;
    g.circle(X(-mw * 0.15, -mw * 0.5), Y(-mw * 0.15, -mw * 0.5), er);
    g.fill({ color: 0x05070b, alpha: 0.95 });
    if (arousal > 0.15) {
      g.circle(X(-mw * 0.15, -mw * 0.5), Y(-mw * 0.15, -mw * 0.5), er);
      g.stroke({
